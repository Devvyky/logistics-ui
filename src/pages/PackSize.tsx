...
import EditProduct from './EditProductSize';
...
const PackSize = () => {
  ...
  return (
    <div className='mx-8 mt-10 mb-6'>
      {modalVisible && <EditProduct setModalVisible={setModalVisible} />}
      <div className='pb-6 relative'>
        <button onClick={() => navigate(-1)} className='absolute top-0 left-[13.3%] font-medium'>
          &#8592; Back
        </button>
            ...
            <table className='w-4/5 mx-auto'>
              ...
              <tbody>
                {data?.map((product: PackSizeProps, index: number) => (
                  <tr key={product?.id} className='text-center border-b border-b-neutral-300'>
                    ...
                    <td className='py-4'>
                      <div className='flex justify-evenly'>
                        <button className='bg-black text-white rounded px-2 py-1' onClick={() => setModalVisible(true)}>
                          Edit
                        </button>
                        <button
                          disabled={isPending}
