...
const CreateProduct = ({ setModalVisible }: ModalProps) => {
  ...
  const mutation = useMutation({
    ...
    mutationFn: (formValues: CreateProductProps) => appServices.createProducts(formValues),
  ...
  return (
    <div className='fixed h-full w-full top-0 left-0 bg-black bg-opacity-80 z-50'>
      <div className='px-8 pt-6 relative pb-8 rounded-md max-w-lg mx-auto mt-24 bg-neutral-100 shadow-md'>
        <h2 className='font-bold text-2xl text-center pb-6'>Create Product</h2>
        <button onClick={() => setModalVisible(false)} className='text-4xl absolute top-4 right-6'>
          &times;
        </button>
        <form action='' onSubmit={handleCreate} className='flex flex-col justify-center'>
          <div className='pb-4'>
          ...
          <div className='pb-4'>
            <label htmlFor='pack_size' className='block font-medium pb-1 text-gray-800'>
              Pack size
            </label>
            <input
              type='text'
