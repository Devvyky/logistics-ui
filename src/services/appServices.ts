...
const appServices = {
  async createProducts(body: { product_line: string; pack_size: number }) {
    return request.post('/products', body).then(async ({ data }) => data);
  },
  async listPackSizes(product_line: string) {
    return request.get(`/products?product_line=${product_line}`).then(async ({ data }) => data);
  },
  async getPackSize(id: string) {
    return request.get(`/products/${id}`).then(async ({ data }) => data);
  },
  async editPackSize(id: string, body: { product_line: string; pack_size: number }) {
